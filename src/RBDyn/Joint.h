 ...
 */
template <typename T> Eigen::Matrix3<T> QuatToE(const std::vector<T> &q);
 ...
 * motion
 * subspace.
 * WARNING : Free joint translation parameters are in FP coordinate (Example 4.5
 * p81).
 */
class Joint {
public:
...
public:
   ...
   */
  template <typename T> sva::PTransform<T> pose(const std::vector<T> &q) const;
...
Joint::tanAccel(const std::vector<double> &alphaD) const {
  using namespace Eigen;
  using namespace sva;
  switch (type_) {
  case Rev:
  ...
  case Free:
    return MotionVecd(S_ *
                      (Vector6d() << alphaD[0], alphaD[1], alphaD[2], alphaD[3],
                       alphaD[4], alphaD[5])
                          .finished());
  case Fixed:
  default:
...
inline sva::MotionVecd
Joint::sTanAccel(const std::vector<double> &alphaD) const {
