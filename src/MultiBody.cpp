...
MultiBody::MultiBody(std::vector<Body> bodies, std::vector<Joint> joints,
                     ...
                     std::vector<int> parent, std::vector<sva::PTransformd> Xto)
    : bodies_(std::move(bodies)), joints_(std::move(joints)),
      pred_(std::move(pred)), succ_(std::move(succ)),
      parent_(std::move(parent)), Xt_(std::move(Xto)),
      jointPosInParam_(joints_.size()), jointPosInDof_(joints_.size()),
