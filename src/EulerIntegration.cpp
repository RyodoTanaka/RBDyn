...
/** Compute the sum of the first terms of the Magnus expansion of \Omega such
 * that
 * q' = q*exp(\Omega) is the quaternion obtained after applying a constant
 * acceleration
 * rotation wD for a duration step, while starting with a velocity w
 */
Eigen::Vector3d magnusExpansion(const Eigen::Vector3d &w,
                                const Eigen::Vector3d &wD, double step) {
...
void eulerJointIntegration(Joint::Type type, const std::vector<double> &alpha,
  ...
  case Joint::Spherical: {
    ...
    qi *= qexp;
    qi.normalize(); // This step should not be necessary but we keep it for
                    // robustness
...
void eulerIntegration(const MultiBody &mb, MultiBodyConfig &mbc, double step) {
  ...
  for (std::size_t i = 0; i < joints.size(); ++i) {
    eulerJointIntegration(joints[i].type(), mbc.alpha[i], mbc.alphaD[i], step,
                          mbc.q[i]);
