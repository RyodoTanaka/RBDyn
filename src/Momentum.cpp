...
CentroidalMomentumMatrix::CentroidalMomentumMatrix()
    : cmMat_(), cmMatDot_(), jacVec_(), blocksVec_(), jacWork_(),
      bodiesWeight_() {}

CentroidalMomentumMatrix::CentroidalMomentumMatrix(const MultiBody &mb)
    : cmMat_(6, mb.nrDof()), cmMatDot_(6, mb.nrDof()), jacVec_(mb.nrBodies()),
      blocksVec_(mb.nrBodies()), jacWork_(mb.nrBodies()),
      bodiesWeight_(mb.nrBodies(), 1.), normalAcc_(mb.nrBodies()) {
  init(mb);
}
...
void CentroidalMomentumMatrix::computeMatrix(const MultiBody &mb,
  ...
  for (int i = 0; i < mb.nrBodies(); ++i) {
    ...
    Matrix6d proj =
        bodiesWeight_[i] * jacProjector(X_i_com, bodies[i].inertia());
...
void CentroidalMomentumMatrix::computeMatrixDot(const MultiBody &mb,
  ...
  for (int i = 0; i < mb.nrBodies(); ++i) {
    ...
    Matrix6d projDot =
        bodiesWeight_[i] *
        jacProjectorDot(X_i_com, bodies[i].inertia(), mbc.bodyVelB[i], com_Vel);
...
void CentroidalMomentumMatrix::computeMatrixAndMatrixDot(
...
sva::ForceVecd
CentroidalMomentumMatrix::momentum(const MultiBody &mb,
                                   const MultiBodyConfig &mbc,
  ...
  for (int i = 0; i < mb.nrBodies(); ++i) {
    ...
    // momentum at CoM for link i : {}^iX_{com}^T {}^iI_i {}^iV_i
    cm +=
        ((mbc.bodyPosW[i] * X_com_0).transMul(hi).vector()) * bodiesWeight_[i];
  }
...
void CentroidalMomentumMatrix::sComputeMatrixDot(
    const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &com,
    const Eigen::Vector3d &comDot) {
  checkMatchBodyPos(mb, mbc);
  checkMatchBodyVel(mb, mbc);
...
void CentroidalMomentumMatrix::sComputeMatrixAndMatrixDot(
    const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &com,
    const Eigen::Vector3d &comDot) {
  checkMatchBodyPos(mb, mbc);
  checkMatchBodyVel(mb, mbc);
...
sva::ForceVecd
CentroidalMomentumMatrix::sMomentum(const MultiBody &mb,
                                    const MultiBodyConfig &mbc,
                                    const Eigen::Vector3d &com) const {
  checkMatchBodyPos(mb, mbc);
  checkMatchBodyVel(mb, mbc);
...
sva::ForceVecd CentroidalMomentumMatrix::sNormalMomentumDot(
    const MultiBody &mb, const MultiBodyConfig &mbc, const Eigen::Vector3d &com,
    const Eigen::Vector3d &comDot) {
  checkMatchBodyPos(mb, mbc);
  checkMatchBodyVel(mb, mbc);
