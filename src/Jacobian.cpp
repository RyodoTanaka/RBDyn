...
MultiBody Jacobian::subMultiBody(const MultiBody &mb) const {
  ...
  return MultiBody(std::move(bodies), std::move(joints), std::move(pred),
                   std::move(succ), std::move(parent), std::move(Xt));
}
...
const Eigen::MatrixXd &Jacobian::bodyJacobian(const MultiBody &mb,
                                              const MultiBodyConfig &mbc) {
  int N = jointsPath_.back();
...
Jacobian::vectorBodyJacobian(const MultiBody &mb, const MultiBodyConfig &mbc,
                             const Eigen::Vector3d &vector) {
  const std::vector<Joint> &joints = mb.joints();
  ...
  for (std::size_t index = 0; index < jointsPath_.size(); ++index) {
    ...
    for (int dof = 0; dof < joints[i].dof(); ++dof) {
      jac_.col(curJ + dof).tail<3>().noalias() =
          X_i_N.rotation() *
          (diff.cross(mbc.motionSubspace[i].col(dof).head<3>()));
    }
...
const Eigen::MatrixXd &Jacobian::jacobianDot(const MultiBody &mb,
                                             const MultiBodyConfig &mbc) {
  const std::vector<Joint> &joints = mb.joints();
  ...
  sva::PTransformd E_N_0(
      Eigen::Matrix3d(mbc.bodyPosW[N].rotation().transpose()));
  ...
  for (std::size_t index = 0; index < jointsPath_.size(); ++index) {
    ...
    for (int j = 0; j < joints[i].dof(); ++j) {
      ...
      jacDot_.col(curJ).noalias() = (E_VN.cross(E_N_0 * X_i_Np * S_ij) +
                                     E_N_0 * X_VNp_i_Np.cross(X_i_Np * S_ij))
                                        .vector();
      ++curJ;
    }
  }
...
const Eigen::MatrixXd &Jacobian::bodyJacobianDot(const MultiBody &mb,
                                                 const MultiBodyConfig &mbc) {
  const std::vector<Joint> &joints = mb.joints();
...
sva::MotionVecd Jacobian::normalAcceleration(
...
sva::MotionVecd
Jacobian::bodyNormalAcceleration(const MultiBody & /* mb */,
                                 const MultiBodyConfig &mbc) const {
...
sva::MotionVecd Jacobian::bodyNormalAcceleration(
    const MultiBody & /* mb */, const MultiBodyConfig &mbc,
    const std::vector<sva::MotionVecd> &normalAccB) const {
...
void Jacobian::translateBodyJacobian(
...
Eigen::MatrixXd
Jacobian::expand(const MultiBody &mb,
...
void Jacobian::expandAdd(const MultiBody &mb,
  ...
  for (int i : jointsPath_) {
    ...
    for (int j : jointsPath_) {
      res.block(mb.jointPosInDof(i), mb.jointPosInDof(j), mb.joint(i).dof(),
                mb.joint(j).dof())
          .noalias() +=
...
const Eigen::MatrixXd &Jacobian::sBodyJacobian(const MultiBody &mb,
                                               const MultiBodyConfig &mbc) {
  checkMatchBodyPos(mb, mbc);
  checkMatchMotionSubspace(mb, mbc);
...
void Jacobian::sTranslateJacobian(const Eigen::MatrixXd &jac,
  ...
  if (jac.cols() != jac_.cols() || jac.rows() != jac_.rows()) {
    ...
    str << "jac matrix size mismatch: expected size (" << jac_.rows() << " x "
        << jac_.cols() << ")"
  ...
  if (res.cols() != jac_.cols() || res.rows() != jac_.rows()) {
    std::ostringstream str;
    str << "res matrix size mismatch: expected size (" << jac_.rows() << " x "
        << jac_.cols() << ")"
...
sva::MotionVecd
Jacobian::sNormalAcceleration(const MultiBody &mb,
                              const MultiBodyConfig &mbc) const {
  checkMatchBodyPos(mb, mbc);
  checkMatchBodyVel(mb, mbc);
  checkMatchJointConf(mb, mbc);
...
sva::MotionVecd
Jacobian::sBodyNormalAcceleration(const MultiBody &mb,
                                  const MultiBodyConfig &mbc) const {
  checkMatchJointConf(mb, mbc);
  checkMatchParentToSon(mb, mbc);
...
sva::MotionVecd Jacobian::sNormalAcceleration(
    const MultiBody &mb, const MultiBodyConfig &mbc,
    const std::vector<sva::MotionVecd> &normalAccB) const {
  checkMatchBodyPos(mb, mbc);
  checkMatchBodyVel(mb, mbc);
...
sva::MotionVecd Jacobian::sBodyNormalAcceleration(
    const MultiBody &mb, const MultiBodyConfig &mbc,
    const std::vector<sva::MotionVecd> &normalAccB) const {
  checkMatchBodiesVector(mb, normalAccB, "normalAccB");
...
sva::MotionVecd Jacobian::normalAcceleration(
    const MultiBodyConfig &mbc, const sva::MotionVecd &bodyNNormalAcc,
    const sva::PTransformd &X_b_p, const sva::MotionVecd &V_b_p) const {
  int N = jointsPath_.back();
...
sva::MotionVecd
Jacobian::normalAcceleration(const MultiBodyConfig &mbc,
...
sva::MotionVecd
Jacobian::bodyNormalAcceleration(const MultiBodyConfig & /* mbc */,
                                 const sva::MotionVecd &bodyNNormalAcc) const {
