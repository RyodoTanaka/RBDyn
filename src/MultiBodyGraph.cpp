...
MultiBodyGraph::MultiBodyGraph(const std::string rootName)
    : rootJointName_(rootName), nodes_(), joints_(), bodyNameToNode_(),
      jointNameToJoint_() {}

MultiBodyGraph::MultiBodyGraph(const MultiBodyGraph &mbg) { copy(mbg); }

MultiBodyGraph::~MultiBodyGraph() { clear(); }
...
std::size_t MultiBodyGraph::nrNodes() const { return nodes_.size(); }

std::size_t MultiBodyGraph::nrJoints() const { return joints_.size(); }
...
MultiBody MultiBodyGraph::makeMultiBody(const std::string &rootBodyName,
  ...
  std::function<void(const std::shared_ptr<Node> curNode,
                     ...
                     int p, int s, int par, const sva::PTransformd &Xt,
                     const sva::PTransformd &Xbase)>
      makeTree;

  makeTree = [&](const std::shared_ptr<Node> curNode,
                 const std::shared_ptr<Node> fromNode, const Joint &joint,
                 int p, int s, int par, const sva::PTransformd &Xti,
                 const sva::PTransformd &Xbase) {
...
void MultiBodyGraph::mergeSubBodies(
...
MultiBodyGraph
MultiBodyGraph::fixJoints(const MultiBodyGraph &other,
...
bool MultiBodyGraph::findMergeSubNodes(
  ...
  for (auto it = node.arcs.begin(); it != node.arcs.end(); ++it) {
    if (it->joint.name() != parentJointName) {
      if (it->joint.name() == jointName) {
        ...
        sva::RBInertiad newInertia =
            mergeInertia(node.body.inertia(),
        ...
        if (findMergeSubNodes(*it->next, it->joint.name(), jointName,
                              jointPosByName)) {
          return true;
        }
      }
...
sva::RBInertiad MultiBodyGraph::mergeSubNodes(
    Node &node, const std::string &parentJointName,
    const std::map<std::string, std::vector<double>> &jointPosByName) {
  ...
  for (const Arc &arc : node.arcs) {
    if (arc.joint.name() != parentJointName) {
      newInertia =
          mergeInertia(newInertia, mergeSubNodes(*arc.next, arc.joint.name(),
                                                 jointPosByName),
...
sva::RBInertiad MultiBodyGraph::mergeInertia(
  ...
  if (jointPosByName.find(joint.name()) == jointPosByName.end()) {
    std::ostringstream msg;
    msg << "jointPosByName  must contain joint " << joint.name()
        << " configuration";
  ...
  // set merged sub inertia in current body base and add it to the current
  // inertia
