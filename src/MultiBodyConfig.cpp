...
MultiBodyConfig::MultiBodyConfig(const MultiBody &mb)
    : q(mb.nrJoints()), alpha(mb.nrJoints()), alphaD(mb.nrJoints()),
      force(mb.nrBodies()), jointConfig(mb.nrJoints()),
      jointVelocity(mb.nrJoints()), jointTorque(mb.nrJoints()),
      motionSubspace(mb.nrJoints()), bodyPosW(mb.nrBodies()),
      parentToSon(mb.nrBodies()), bodyVelW(mb.nrBodies()),
      bodyVelB(mb.nrBodies()), bodyAccB(mb.nrBodies()), gravity(0., 9.81, 0.) {
...
void MultiBodyConfig::python_motionSubspace(
...
ConfigConverter::ConfigConverter(const MultiBody &from, const MultiBody &to)
    : jInd_(from.nrJoints() - 1), bInd_(from.nrBodies()) {
...
void ConfigConverter::convert(const MultiBodyConfig &from,
                              MultiBodyConfig &to) const {
...
ConfigConverter *ConfigConverter::sConstructor(const MultiBody &from,
                                               const MultiBody &to) {
  bool isOk = true;
...
void ConfigConverter::sConvert(const MultiBodyConfig &from,
                               MultiBodyConfig &to) const {
  bool isOk = true;
...
std::vector<std::vector<double>> sVectorToParam(const MultiBody &mb,
  ...
  if (mb.nrParams() != e.rows()) {
    std::ostringstream str;
    str << "Parameter vector size mismatch: expected size " << mb.nrParams()
        << " gived " << e.rows();
    throw std::out_of_range(str.str());
  }
...
std::vector<std::vector<double>> sVectorToDof(const MultiBody &mb,
  ...
  if (mb.nrDof() != e.rows()) {
    std::ostringstream str;
    str << "Dof vector size mismatch: expected size " << mb.nrDof() << " gived "
        << e.rows();
    throw std::out_of_range(str.str());
  }
...
void checkMatchJointTorque(const MultiBody &mb, const MultiBodyConfig &mbc) {
  ...
  for (int i = 0; i < static_cast<int>(mbc.jointTorque.size()); ++i) {
    if (mbc.jointTorque[i].size() !=
        static_cast<std::size_t>(mb.joint(i).dof())) {
      std::ostringstream str;
      str << "Bad number of torque variable for Joint " << mb.joint(i)
          << " at position " << i << ": expected size " << mb.joint(i).dof()
          << " gived " << mbc.jointTorque[i].size();
...
void checkMatchMotionSubspace(const MultiBody &mb, const MultiBodyConfig &mbc) {
  ...
  for (int i = 0; i < static_cast<int>(mbc.motionSubspace.size()); ++i) {
    if (mbc.motionSubspace[i].cols() !=
        static_cast<unsigned>(mb.joint(i).dof())) {
      std::ostringstream str;
      str << "Bad motionSubspace matrix size for Joint " << mb.joint(i)
          << " at position " << i << ": expected column number "
...
void checkMatchAlphaD(const MultiBody &mb, const MultiBodyConfig &mbc) {
  checkMatchJointsVector(mb, mbc.alphaD,
                         "Generalized acceleration variable vector");
