...
InverseStatics::InverseStatics(const MultiBody &mb)
    : f_(mb.nrBodies()), df_(mb.nrBodies()), jointTorqueDiff_(mb.nrJoints()),
      jacW_(mb.nrBodies()), fullJac_(6, mb.nrDof()),
...
void InverseStatics::computeTorqueJacobianJoint(
  ...
  for (std::size_t i = 0; i < bodies.size(); ++i) {
    ...
    df_[i].block(0, 0, fullJac_.rows(), fullJac_.cols()) =
        (bodies[i].inertia().matrix() * M - N) * fullJac_;
...
void InverseStatics::computeTorqueJacobianJoint(const MultiBody &mb,
  ...
  for (int i = 0; i < mb.nrJoints(); ++i)
    jacMandF.push_back(MatrixXd(0, 0));
...
void InverseStatics::sInverseStatics(const MultiBody &mb,
                                     MultiBodyConfig &mbc) {
