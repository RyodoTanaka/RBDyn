...
BOOST_AUTO_TEST_CASE(MultiBodyGraphTest) {
  ...
  // name already exists
  BOOST_CHECK_THROW(mbg1.addJoint(Joint(Joint::RevX, true, "j3")),
                    std::domain_error);
  ...
  BOOST_CHECK_NO_THROW(mbg1.linkBodies("b1", PTransformd::Identity(), "b2",
                                       PTransformd::Identity(), "j1"));
  BOOST_CHECK_NO_THROW(mbg1.linkBodies("b1", PTransformd::Identity(), "b3",
                                       PTransformd::Identity(), "j2"));
  BOOST_CHECK_NO_THROW(mbg1.linkBodies("b3", PTransformd::Identity(), "b4",
                                       PTransformd::Identity(), "j3"));

  // check non-existant body 1
  BOOST_CHECK_THROW(mbg1.linkBodies("b10", PTransformd::Identity(), "b4",
                                    PTransformd::Identity(), "j3"),
                    std::out_of_range);
  // check non-existant body 2
  BOOST_CHECK_THROW(mbg1.linkBodies("b3", PTransformd::Identity(), "b10",
                                    PTransformd::Identity(), "j3"),
                    std::out_of_range);
  // check non-existant joint
  BOOST_CHECK_THROW(mbg1.linkBodies("b3", PTransformd::Identity(), "b4",
                                    PTransformd::Identity(), "j10"),
                    std::out_of_range);
}
...
void checkMultiBodyNames(const rbd::MultiBody &mb,
  ...
  for (const rbd::Body &b : mb.bodies()) {
    BOOST_CHECK(std::find(bodies.begin(), bodies.end(), b.name()) !=
                bodies.end());
  }
  for (const rbd::Joint &j : mb.joints()) {
    BOOST_CHECK(std::find(joints.begin(), joints.end(), j.name()) !=
                joints.end());
  }
}
...
void checkMultiBodyEq(const rbd::MultiBody &mb, std::vector<rbd::Body> bodies,
                      std::vector<rbd::Joint> joints, std::vector<int> pred,
                      std::vector<int> succ, std::vector<int> parent,
  ...
  // pred
  BOOST_CHECK_EQUAL_COLLECTIONS(mb.predecessors().begin(),
                                mb.predecessors().end(), pred.begin(),
                                pred.end());
  // succ
...
BOOST_AUTO_TEST_CASE(MultiBodyTest) {
  ...
  std::vector<Body> bodies = {Body(r, "b1"), Body(r, "b2"), Body(r, "b3"),
                              Body(r, "b4")};

  std::vector<Joint> joints = {
      Joint(Joint::RevX, true, "j0"), Joint(Joint::RevX, true, "j1"),
  ...
  BOOST_CHECK_THROW(mb.sTransforms({newTrans, newTrans, I}),
                    std::runtime_error);
...
BOOST_AUTO_TEST_CASE(MakeMultiBodyTest) {
  ...
  //   j1  /   j2      j3
  //       b1 ---- b3 ---- b4
  mbg1.linkBodies("b1", PTransformd::Identity(), "b2", PTransformd::Identity(),
                  "j1");
  mbg1.linkBodies("b1", PTransformd::Identity(), "b3", PTransformd::Identity(),
                  "j2");
  mbg1.linkBodies("b3", PTransformd::Identity(), "b4", PTransformd::Identity(),
                  "j3");
  ...
  auto testSucc =
      [](const std::map<std::string, std::vector<std::string>> &s1,
  ...
  std::map<std::string, std::vector<std::string>> succRes3{
      {"b2", {"j1"}}, {"b1", {"j2"}}, {"b3", {"j3"}}, {"b4", {}}};
  std::map<std::string, std::string> predRes3{
      {"b2", "Root"}, {"b1", "j1"}, {"b3", "j2"}, {"b4", "j3"}};
  ...
  mbg2.linkBodies("b1", PTransformd(Vector3d(Vector3d::UnitX())), "b2",
                  PTransformd(Vector3d(-Vector3d::UnitY())), "j1");
  mbg2.linkBodies("b2", PTransformd(Vector3d(Vector3d::UnitX())), "b3",
                  PTransformd(Vector3d(-Vector3d::UnitZ())), "j2");
  mbg2.linkBodies(
      "b2", PTransformd(Matrix3d(sva::RotX(constants::pi<double>() / 2.))),
  ...
  Xt = {root, PTransformd(Vector3d(1., 0., 0.)),
  ...
  // check body com
  std::vector<Vector3d> bCom = {Vector3d::Zero(), Vector3d(0., 1., 0.),
...
BOOST_AUTO_TEST_CASE(MultiBodyConfigFunction) {
  ...
  for (std::size_t i = 0; i < v1.size(); ++i) {
    BOOST_CHECK_EQUAL_COLLECTIONS(v1[i].begin(), v1[i].end(), v2[i].begin(),
                                  v2[i].end());
  }
...
BOOST_AUTO_TEST_CASE(MultiBodyConfigFunction2) {
  ...
  mbg.linkBodies("b0", PTransformd::Identity(), "b1", PTransformd::Identity(),
                 "j0");
  mbg.linkBodies("b1", PTransformd::Identity(), "b2", PTransformd::Identity(),
                 "j1");
  mbg.linkBodies("b2", PTransformd::Identity(), "b3", PTransformd::Identity(),
                 "j2");
  ...
  for (int i = 0; i < mb.nrJoints(); ++i) {
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(mbc.q[i].begin(), mbc.q[i].end(), zp.begin(),
                                  zp.end());
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(mbc.alphaD[i].begin(), mbc.alphaD[i].end(),
                                  zd.begin(), zd.end());
    BOOST_CHECK_EQUAL_COLLECTIONS(mbc.jointTorque[i].begin(),
                                  mbc.jointTorque[i].end(), zd.begin(),
                                  zd.end());
  }
...
void checkConfig(const rbd::MultiBodyConfig &mbc1,
                 const rbd::MultiBodyConfig &mbc2) {
...
BOOST_AUTO_TEST_CASE(MultiBodyBaseTransformTest) {
  ...
  mb3 = mbg.makeMultiBody("b3", true,
                          mb3Surface * mb0ToBase["b3"] *
                              mbc0.bodyPosW[mb0.bodyIndexByName("b3")],
                          mb3Surface);
  ...
  mb4 = mbg.makeMultiBody("b4", true,
                          mb4Surface * mb0ToBase["b4"] *
                              mbc0.bodyPosW[mb0.bodyIndexByName("b4")],
                          mb4Surface);
  ...
  for (int bi = 0; bi < mb0.nrBodies(); ++bi) {
    ...
    BOOST_CHECK_SMALL(((mb0ToBase[name] * mbc0.bodyPosW[bi]).matrix() -
                       (mb3ToBase[name] * mbc3.bodyPosW[mb3Index]).matrix())
                          .norm(),
                      1e-8);
    BOOST_CHECK_SMALL(((mb0ToBase[name] * mbc0.bodyPosW[bi]).matrix() -
                       (mb4ToBase[name] * mbc4.bodyPosW[mb4Index]).matrix())
                          .norm(),
                      1e-8);
  }
}
