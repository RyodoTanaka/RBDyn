...
std::vector<double> ZeroParam(Joint::Type t) { return Joint::ZeroParam(t); }

std::vector<double> ZeroDof(Joint::Type t) { return Joint::ZeroDof(t); }

ConfigConverter *ConfigConverterConstructor(const MultiBody &mb1,
                                            const MultiBody &mb2) {
...
void dvv_set_item(std::vector<std::vector<double>> &v, unsigned int idx,
                  const std::vector<double> &value) {
  v[idx] = value;
}

void mbcv_set_item(std::vector<MultiBodyConfig> &v, unsigned int idx,
